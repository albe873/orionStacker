...
Mat to_bgr_mat_from_planar_data(uint16_t *image_data, long width, long height) {
...
void save_image_fits(string output_dir_path, string file_name, float *image_data, long width, long height, long n_chan) {
    ...
    // Write image data
    if (fits_write_img(fptr, TFLOAT, 1, width * height * n_chan, image_data, &status)) {
        fits_report_error(stderr, status);
        return;
    }
...
int load_images_to_memory(const char *dir_path, uint16_t *img_all, long width, long height, long n_chan, int count, double *timestamps) {
    DIR *dir = opendir(dir_path);
    if (!dir) {
        perror("opendir");
        return 1;
